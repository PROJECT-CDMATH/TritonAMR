      subroutine solver_vfin_cgradg_elli(isit,indf,ri,rj,rk, &
      ...
      integer isit, indf,ri,rj,rk, &
        ...
        ndimc,ndimf,neqt, nfic
	integer ijfmax, ijkfmax, ijk0, ijk1, ii, jj,kk,nne
...
! calcul des valeurs fines fictives 
        ...
        do j=1 ,jfmax(ifc)
        do i=1, ifmax(ifc)
          ii = (ri-i+1)*(1-ib1) + i*(1+ib1) -1
          jj = (rj-j+1)*(1-ib2) + j*(1+ib2) -1
          kk = (rk-k+1)*(1-ib3) + k*(1+ib3) -1
...
!    &         + c2* r(ijkc,ne) &
               + c2* (r(ijkc,ne) + xi*slopei+yj*slopej+zk*slopek )
        enddo	  
        enddo	  
        enddo	
