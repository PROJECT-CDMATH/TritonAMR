...
!		amr driver							      *
!                                                                                     *
! SYNTAXE SUBROUTINE:  subroutine amr_driv_uns_solver_meth_1                          *
!										      *
!                         methode 1 : x____y___z - x____y____z                        *
!										      *
!**************************************************************************************

subroutine amr_driv_uns_solver_meth_1
...
!!!!!!!!!!!!! DEBUT DE DECLARATIONS DES VARIABLES LOCALES !!!!!!!!!!!!!!!
	...
	integer lpr, l, ln, lmax_exist, k,  ll, num, idir  
	character*3 cnum
	character*6 chfich
...
! --- for level 0 ------------------------
	if (iter.lt.itamr.or.lmax.eq.0) then
		if(iter.eq.0) call amr_plt_paraview_NODE(0.,0)
...
! --- for level 1 ------------------------
		if (iter.eq.itamr) then
			...
			if(dir(1).eq.2.AND.iter.EQ.0) call amr_linit_2D(1)			
			if(dir(1).eq.3.AND.iter.EQ.0) call amr_linit_3D(1)
			ELSE IF(INIT_BULLE_USER.EQ.1) THEN
			IF(iter.EQ.0)	CALL US_INIT_BULLE(1)
			END IF
			...
			if (mod(iter,itgfr).eq.0)  then
				...
				call amr_trans(0,1)
				call amr_trans(1,1)
				IF(INIT_BULLE_USER.EQ.0.AND.ELL_YES.EQ.0) THEN
				if(dir(1).eq.2.AND.iter.EQ.1) call amr_linit_2D(1)
				if(dir(1).eq.3.AND.iter.EQ.1) call amr_linit_3D(1)
				ELSE IF(INIT_BULLE_USER.EQ.1.AND.ELL_YES.EQ.0) THEN
				IF(iter.EQ.1) CALL US_INIT_BULLE(1)
				END IF
			end if
				...
				if(mise_jour_adv.NE.1)	CALL US_INIT_ADVECTION(1)
				END IF
				call l_amr_ghost_fc(1)
				do idir=1,dir(1)	
					TYPE_DIR = dir(idir+1)
