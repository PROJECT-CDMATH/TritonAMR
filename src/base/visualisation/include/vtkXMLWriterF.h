...
# define VTK_FORTRAN_NAME(name, NAME) name##_
#endif
...
void VTK_FORTRAN_NAME(vtkxmlwriterf_setdataobjecttype, VTKXMLWRITERF_SETDATAOBJECTTYPE)(
	const int* self, const int* objType, const int* writetype
  )
{
  if(*self > 0 && *self <= VTK_XMLWRITERF_MAX && vtkXMLWriterF_Table[*self])
    {
      vtkXMLWriterC_SetDataObjectType(vtkXMLWriterF_Table[*self], *objType, *writetype);
    }
  else
    {
    fprintf(stderr,
