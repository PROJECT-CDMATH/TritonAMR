...
!!!!!!!!!!!!! FIN DE DECLARATIONS DES VARIABLES LOCALES !!!!!!!!!!!!!!!!!
	...
	IF(num_Y.eq.0) THEN 
	cnum_Y='0000'
	OPEN(NFSOLYPVD,FILE='Sol'//cnum_Y//'_Y.pvd')
	...
	cnum_vitesse='0000'
	OPEN(NFSOLVPVD,FILE='Sol'//cnum_vitesse//'_vitesse.pvd')
...
!c     calcul du pourcentage de raffinement:
	...
	OPEN(NFSOLYVTR,FILE='TritonAmr'//cnum_Y//'_'//nim//'_Y.vtr')
		
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")"<?xml version=""1.0""?>"
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")"<VTKFile type=""RectilinearGrid"" version=""0.1"""
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")" byte_order=""LittleEndian"" compressor=""vtkZLibDataCompressor"">"
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")'<RectilinearGrid WholeExtent="'
        WRITE(NFSOLYVTR,*) extent(1), extent(2), extent(3), extent(4), extent(5), extent(6)
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") '">'
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no") '<Piece Extent="'
        WRITE(NFSOLYVTR,*) extent(1), extent(2), extent(3), extent(4), extent(5), extent(6)
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") '">'
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")'<PointData Scalars="champs">'
	WRITE(NFSOLYVTR,"(A)",ADVANCE="no")'<DataArray type="Float64" Name="champ" FORMAT="ascii">'
        
        WRITE(NFSOLYVTR,*) YY_nodes_1D
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "</DataArray></PointData><CellData></CellData><Coordinates>"
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "<DataArray type=""Float64"" Name=""Array 0x829d368"" FORMAT=""ascii"">"
        WRITE(NFSOLYVTR,*) XXX
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "</DataArray>"
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "<DataArray type=""Float64"" Name=""Array 0x829c5c0"" FORMAT=""ascii"">"
        WRITE(NFSOLYVTR,*) YYY
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "</DataArray>"
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "<DataArray type=""Float64"" Name=""Array 0x829c5c0"" FORMAT=""ascii"">"
        WRITE(NFSOLYVTR,*) ZZZ
        WRITE(NFSOLYVTR,"(A)",ADVANCE="no") "</DataArray></Coordinates></Piece></RectilinearGrid></VTKFile>"
