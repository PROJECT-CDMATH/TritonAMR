...
! BUT FONCTION :
! --------------
! ROUTINE UTILISATEUR : DEFINITION DU CRITERE DE RAFFINEMENT 2D
...
! VARIABLES LOCALES :
! -------------------
	INTEGER M, IM, IJK, IBMP, JBMP, KBMP, IJK0, i, j, ISENSOR1
	
	DOUBLE PRECISION EPS, GRADX_Y, GRADY_Y, GRAD_MAX_Y, SENSOR1
!*******************************************************************
		...
		DO j = 1,jmx(IM)
			...
			DO i = 1,imx(IM)
				...
				GRADY_Y = q(IJK+JBMP,1)-q(IJK-JBMP,1)
					GRAD_MAX_Y = Max(GRAD_MAX_Y,abs(GRADX_Y)+abs(GRADY_Y))
					IJK = IJK + IBMP
			END DO
				IJK = IJK0 + JBMP
		END DO
	...
	DO M = 1, nga(l)
		...
		DO j = 1,jmx(IM)
			...
			DO i = 1,imx(IM)
				GRADX_Y = q(IJK+IBMP,1)-q(IJK-IBMP,1)
				GRADY_Y = q(IJK+JBMP,1)-q(IJK-JBMP,1)
				SENSOR1 = (abs(GRADX_Y )+abs(GRADY_Y ))/GRAD_MAX_Y
